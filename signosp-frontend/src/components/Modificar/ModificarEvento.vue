    ...
    <div v-if="visible">
        <form @submit.prevent="modificar">
            ...
            <div class="submitcontainer">
                <input class="buttonclassic" type="submit" value="Guardar Cambios">
            </div>
        </form>
    </div>
</template>
...
const modificar = async () => {
    try {
        evento.value.id_evento = props.id;
        evento.value.empleados = selectedEmpleados.value;
        evento.value.cliente.id_cliente = selectedCliente.value == 0 ? null : selectedCliente.value;
        ...
        await axios.put(`http://localhost:8080/api/evento`, evento.value);
        modEvent.value.titulo = evento.value.titulo;
        modEvent.value.fecha = evento.value.fecha;
        modEvent.value.id_categoria = evento.value.id_categoria
        emits('update', modEvent);
